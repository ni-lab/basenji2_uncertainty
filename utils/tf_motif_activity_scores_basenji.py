...
def main():
    ...
    tfs = os.listdir(f"{motif_fasta_folder}/shuffled_peaks_basenji_motifs/")
    ...
    fasta_bg = "%s/endogenous_sequences_basenji.fasta" % motif_fasta_folder
    pred_bg = pred_on_fasta(fasta_bg, seqnn_model, n=100)

    tf_activity_scores = pd.DataFrame([], index=tfs, columns=targets["description"].values)
    mutated_tf_activity_scores = pd.DataFrame([], index=tfs, columns=targets["description"].values)
    for tf in tqdm(tfs):
        tf_scores, mutated_tf_scores = motif_score(tf, seqnn_model, motif_fasta_folder, pred_bg, n=100)
        tf_activity_scores.loc[tf] = tf_scores
        mutated_tf_activity_scores.loc[tf] = mutated_tf_scores

    tf_activity_scores.to_csv(f"{options.out_dir}/endogenous_sequence_tf_activity_scores.tsv", sep="\t", header=True, index=True)
    mutated_tf_activity_scores.to_csv(f"{options.out_dir}/endogenous_sequence_mutated_tf_activity_scores.tsv", sep="\t", header=True, index=True)



def pred_on_fasta(fa, model, batch_size=4, n=None):
    ...
    pred = pred[:,447:449,:].sum(axis=1)  # take predictions for central 2 bins because the motif is inserted at the junction
    return pred
...
def motif_score(tf, model, motif_fasta_folder, pred_bg, n=None):
    ...
    """
    fasta_motif = "%s/endogenous_sequences_basenji_motifs/%s.fasta" % (motif_fasta_folder, tf)
    fasta_mutated_motif = "%s/endogenous_sequences_basenji_motifs_mutated/%s.fasta" % (motif_fasta_folder, tf)

    pred_motif = pred_on_fasta(fasta_motif, model, n=n)
    pred_motif_mutated = pred_on_fasta(fasta_mutated_motif, model, n=n)

    tf_score = pred_motif.mean(axis=0) - pred_bg.mean(axis=0)
    mutated_tf_score = pred_motif_mutated.mean(axis=0) - pred_motif.mean(axis=0)
    
    return tf_score, mutated_tf_score
